...
const beltLevels = [
  { title: 'Apprentice', belt: 'Gray',  basePay: '$20–$25/hr',  tenure: '0–2 yrs' },
  { title: 'Junior Tech', belt: 'Blue',  basePay: '$25–$29/hr',  tenure: '2–4 yrs' },
  { title: 'Lead Tech',   belt: 'Green', basePay: '$30–$35/hr',  tenure: '4–7 yrs' },
  { title: 'Senior Tech', belt: 'Brown', basePay: '$36–$41/hr',  tenure: '7–10 yrs' },
  { title: 'Master Tech', belt: 'Black', basePay: '$42+/hr',     tenure: '10+ yrs' },
];
...
export default function CommercialPresentation({ setActiveTab }) {
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="rounded-2xl px-6 py-8 text-center" style={{ background: 'linear-gradient(135deg, #0d2b4e, #0a3d1f)', border: '2px solid #8dc63f40' }}>
        ...
        <p className="text-slate-300 text-sm max-w-xl mx-auto">
          Your hourly pay is <span className="text-white font-bold">guaranteed</span> every week. On top of that, earn up to <span className="text-white font-bold">$3,000/mo in bonuses</span> based on your focus area — Service, Install, or Entry.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {focuses.map((f) => (
            <div key={f.name} className="rounded-xl overflow-hidden" style={{ border: `1px solid ${f.color}30` }}>
              <div className="px-4 py-3 text-center font-black tracking-wide uppercase text-sm" style={{ background: `${f.color}22`, color: f.color, fontFamily: "'Barlow Condensed', sans-serif", fontSize: 16 }}>
                {f.name}
              </div>
              <div className="divide-y" style={{ divideColor: 'rgba(255,255,255,0.05)' }}>
                {f.bonuses.map((b) => (
                  <div key={b.metric} className="px-4 py-3 flex justify-between items-center">
                    <div>
                      <p className="text-white text-sm font-semibold">{b.metric}</p>
                      <p className="text-xs" style={{ color: f.color }}>{b.goal}</p>
                    </div>
                    <span className="font-black text-sm" style={{ color: f.color }}>{b.bonus}</span>
                  </div>
                ))}
              </div>
              <div className="px-4 py-3 text-center text-xs font-bold" style={{ background: `${f.color}10`, color: f.color }}>
                Max $3,000/mo
              </div>
            </div>
          ))}
        </div>
        <p className="text-xs text-slate-500 mt-4 text-center">Each bonus goal is independent — hitting one or two still earns that payout. You don't need all three.</p>
      </Card>
