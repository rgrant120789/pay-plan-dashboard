...
function Card({ title, accent = '#3b82f6', children }) {
  return (
...
export default function ResidentialInstallPresentation({ setActiveTab }) {
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      {/* Hero */}
      <div className="rounded-2xl px-6 py-8 text-center" style={{ background: 'linear-gradient(135deg, #0d2b4e, #0a1f3d)', border: '2px solid #3b82f640' }}>
        <p className="text-xs font-bold tracking-widest uppercase mb-2" style={{ color: '#3b82f6' }}>2026 Pay Plan</p>
      ...
      {/* How pay works */}
      <Card title="How Your Pay Works" accent="#3b82f6">
        <div className="space-y-4">
          ...
          <div className="text-center text-2xl font-black" style={{ color: '#3b82f6' }}>+</div>
          <div className="rounded-xl p-4" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.25)' }}>
      ...
      {/* Bonus goals */}
      <Card title="Monthly Bonus Goals" accent="#3b82f6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          {bonuses.map((b) => (
            <div key={b.metric} className="rounded-xl p-5 text-center" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.25)' }}>
              <p className="text-2xl font-black mb-1" style={{ color: '#3b82f6' }}>{b.bonus}</p>
              <p className="text-white font-bold text-sm mb-1">{b.metric}</p>
              <p className="text-xs font-bold mb-2" style={{ color: '#60a5fa' }}>{b.goal}</p>
              <p className="text-slate-400 text-xs">{b.desc}</p>
            </div>
          ))}
        </div>
        <div className="rounded-xl p-4 text-center" style={{ background: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.15)' }}>
          <p className="text-white font-bold">Hit all three = <span style={{ color: '#3b82f6' }}>$3,000 bonus that month</span></p>
          <p className="text-slate-400 text-xs mt-1">Each goal is tracked independently — hitting one or two still pays out.</p>
        </div>
      </Card>

      {/* Spiffs */}
      <Card title="Additional Spiff Opportunities" accent="#3b82f6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: 'Google Review', value: '$15', desc: 'Per verified 5-star review' },
            { label: 'Yard Sign', value: '$5', desc: 'Per yard sign placed' },
            { label: 'Weekly On-Call', value: '$150', desc: 'Per on-call week completed' },
          ].map((s) => (
            <div key={s.label} className="rounded-xl p-4 text-center" style={{ background: 'rgba(59,130,246,0.07)', border: '1px solid rgba(59,130,246,0.2)' }}>
              <p className="text-2xl font-black mb-1" style={{ color: '#3b82f6' }}>{s.value}</p>
      ...
      {/* Belt levels */}
      <Card title="Belt Levels & Base Hourly Ranges" accent="#3b82f6">
      ...
      {/* Skills */}
      <Card title="Skills Required by Belt Level" accent="#3b82f6">
